                          ...
                          enum tpm_duration_type to_t)
{
	...
	succ = spapr_vtpm_transmit((uint8_t *)req,
	                           be32_to_cpu(req->totlen), to_t,
	                           respbuffer, respbufferlen);
	ret = !succ ? -1 : 0;
					   ...
					   uint32_t datalen)
{
	...
	return hash_log_extend(&pcpes, data, datalen,
			       info, infolen, pcrindex, true);
}
