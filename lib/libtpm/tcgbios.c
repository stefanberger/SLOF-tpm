...
static const struct hash_parameters *tpm2_find_by_hashalg(uint16_t hashAlg)
{
	unsigned i;
...
static const struct hash_parameters *
tpm2_find_by_hashalg_flag(uint16_t hashalg_flag)
{
	unsigned i;
...
{
	const struct hash_parameters *hp = tpm2_find_by_hashalg(hashAlg);
...
static uint16_t tpm20_hashalg_flag_to_hashalg(uint8_t hashalg_flag)
{
	...
	hp = tpm2_find_by_hashalg_flag(hashalg_flag);
	if (hp)
		return hp->hashalg;
	return 0;
}
			      ...
			      bool bigEndian)
{
	...
	void *nsel, *end;
	void *dest = le->hdr.digests + sizeof(struct tpm2_digest_values);
	uint32_t count;
