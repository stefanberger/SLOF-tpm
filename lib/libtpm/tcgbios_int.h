...
struct tpm2_digest_values {
	uint32_t count;
	struct tpm2_digest_value digest[0]; /* variable number of entries */
} __attribute__((packed));

/* Each entry in the TPM log contains: a TCG_PCR_EVENT2_Header, a variable
 * length digest, a TCG_PCR_EVENT2_Trailer, and a variable length event.  The
 * 'digest' matches what is sent to the TPM hardware via the Extend
 * command.  on TPM2.0 the digest contains a tpm2_digest_values struct
 * followed by a variable number of tpm2_digest_value structs (as
 * specified by the hardware via the TPM2_CAP_PCRS request).
 */
 ...
 *
 * Relevant specs for #defines:
 * - Trusted Platform Module Library; Part 2: Structures
 * Relevant specs for structures:
